...
//Datos de WiFi
const char* ssid = "Fam. ARCINIEGA M";  // Aquí debes poner el nombre de tu red
...
const int TRIGGER = 14; // Pin de "Trigger", donde se manda la señal ultrasonica.
const int ECHO = 15; // Pin de "Echo#, que se encarga de recibir la señal que rebota en el objeto.
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("codigoiot/distancia/rodrigoarciniega", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "codigoiot/distancia/rodrigoarciniega") {  // En caso de recibirse mensaje en el tema codigoiot/distancia/rodrigoarciniega
    if (messageTemp == "true") {
      Serial.println("Led encendido");
      digitalWrite(flashLedPin, HIGH);
    }// fin del if (String(topic) == "codigoiot/distancia/rodrigoarciniega")
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("codigoiot/distancia/rodrigoarciniega"); // Esta función realiza la suscripción al tema
